...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, stride=1, residual=True):
        ...
        self.A = nn.Parameter(torch.tensor(np.sum(np.reshape(A.astype(np.float32),[3,25,25]),axis=0), dtype=torch.float32,requires_grad=False,device='cuda'),requires_grad=False)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=8, graph=None, graph_args=dict(), in_channels=3):
        ...
        self.l1 = TCN_GCN_unit(3, 64, A, groups, residual=False)
